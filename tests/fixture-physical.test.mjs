...
import { fixtureIpRatings, fixtureQuickSpecs, formatFixtureWattage, kilogramsToPounds } from "../src/lib/fixture-physical.ts";
...
describe("Fixture physical fields", () => {
  ...
  it("accepts optional positive decimal wattage and formats it in watts", () => {
    for (const value of ["", "0.5", "1200", "1200.25"]) assert.equal(validateContentInput("fixture", "contributor", { ...base, wattage: value }).valid, true);
    for (const value of ["0", "-1", "watts", "Infinity"]) assert.equal(validateContentInput("fixture", "contributor", { ...base, wattage: value }).valid, false);
    assert.equal(formatFixtureWattage("1200"), "1200 W");
  });
  ...
  it("keeps manual create/edit, optional old records, links, Typical Use and categories intact", () => {
    ...
    assert.equal(validateContentInput("fixture", "contributor", existing).valid, false);
    const result = validateContentInput("fixture", "contributor", existing, "draft", existing);
    assert.equal(result.valid, true); assert.equal(result.payload.weight_lb, null); assert.equal(result.payload.wattage, null); assert.equal(result.payload.ip_rating, null);
  ...
  it("uses editable fields in both forms and compact detail quick specs with lb units", async () => {
    ...
    assert.match(editor, /\["wattage", "weight_lb"\]\.includes\(field.name\)/);
    assert.match(pages, /Fixture quick specs/); assert.match(pages, /formatFixtureWattage\(stringify\(record\[key\]\)\)/);
    assert.deepEqual(fixtureQuickSpecs, ["preferred_mode", "dmx_footprint", "wattage", "weight_lb", "ip_rating", "power_input_connector"]);
  });
});
