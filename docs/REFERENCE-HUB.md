...
The forward migration `supabase/migrations/202609130002_ldg_documents_collection.sql` adds **LDG / LDGE Documents** (`ldg-ldge-documents`) as a top-level collection. Its description is: “Internal company documents, forms, policies, templates, handbooks, and shared operational resources.” It is transactional and idempotent (`ON CONFLICT (id) DO NOTHING`), preserves existing taxonomy/content, and changes no permissions.

The later forward migration `supabase/migrations/202609140008_retire_fixture_firmware_collection.sql` retires the redundant **Fixtures & Firmware** Reference Hub collection in favor of the dedicated Fixtures module. Existing bookmarks are moved to Unsorted without changing their URLs, tags, revision history, or timestamps; the legacy Notion label also maps to Unsorted. The current taxonomy is **10 top-level collections and three subcollections**.
...
- `/links/social` is separate from reference bookmarks and visible only to active T.I.K.I. members. The follow-up in [SOCIAL-ACCOUNTS.md](SOCIAL-ACCOUNTS.md) adds platform-aware account editing, persistent navigation, and explicit admin management of active members' social accounts through a new policy migration. All active roles, including Viewers, retain owner editing; other members remain read-only. No impersonation is used.
