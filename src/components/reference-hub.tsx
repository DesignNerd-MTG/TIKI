...
export async function ReferenceHub({ params }: { params: ReferenceParams }) {
  ...
  return <div className="page-stack">
    <PageHeader eyebrow="Reference Hub" title={sub?.name ?? selected?.name ?? "Reference Hub"} description={sub?.description ?? selected?.description ?? "Useful references organized by what they help you do."} action={<div className="page-actions">{canCreateContent(profile.role,"link") && <Link className="primary-button" href="/links/new">+ Add Reference</Link>}<Link className="secondary-button" href="/links/social">Social Directory</Link>{profile.role === "admin" && <Link href="/links/import">Notion import</Link>}</div>} />
