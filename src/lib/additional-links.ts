import { isSafeExternalUrl, normalizeExternalUrl } from "./content-validation.ts";
...
export function validateAdditionalLinks(kind: EntityKind, links: AdditionalLink[]) {
  ...
  for (const link of links) {
    ...
    if (link.url.length > 2048 || !isSafeExternalUrl(link.url)) return { valid: false as const, message: "Use a valid web address or http:// or https:// URL for every additional link." };
  }
...
export function readAdditionalLinks(formData: FormData, kind: EntityKind) {
  ...
  const urls = formData.getAll("additional_link_url").map((value) => normalizeExternalUrl(String(value)));
  ...
  const validation = validateAdditionalLinks(kind, links);
  return validation.valid ? validation : { ...validation, links };
}
