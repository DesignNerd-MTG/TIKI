...
export async function updateCustomPaletteAction(_state:PaletteActionState,form:FormData):Promise<PaletteActionState>{
  ...
  if(!["save","reset","activate","preset"].includes(command))return{ok:false,message:"Choose a valid palette action."};
  if(command==="reset") { const removed=await client.from("user_custom_palettes").delete().eq("profile_id",identity.id).eq("slot",slot); if(removed.error)return{ok:false,message:"Palette could not be reset."}; const appearance=await client.from("user_appearance").update({active_custom_slot:null}).eq("profile_id",identity.id).eq("active_custom_slot",slot); if(appearance.error)return{ok:false,message:"The palette reset, but the active appearance could not be cleared."}; revalidatePath("/","layout"); return{ok:true,message:`Custom Palette ${slot} reset.`}; }
  ...
  if(saved.error)return{ok:false,message:"Palette could not be saved."}; revalidatePath("/","layout"); return{ok:true,message:`${name} saved.`};
}
