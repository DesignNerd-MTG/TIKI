...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  if (pattern) {
    ...
    const [fixtures, shows, documents, links, locations, drinks, vendors, napkin, tagLinks] = await Promise.all([
    ...
    searchFailed = [fixtures, shows, documents, links, locations, drinks, vendors, napkin, tagLinks].some((result) => Boolean(result.error));
    groups = [
  ...
  return (
    <div className="page-stack">
      <PageHeader eyebrow="Global search" title={query ? `Results for “${query}”` : "Search T.I.K.I."} description={query ? `${resultCount} result${resultCount === 1 ? "" : "s"} across content and tags.` : "Search fixtures, shows, documents, links, locations, drinks, permitted vendor context, Napkin notes, and tags from one place."} />
