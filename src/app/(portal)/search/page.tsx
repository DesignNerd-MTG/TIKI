...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  if (pattern) {
    ...
    groups = [
      ...
      { label: "Shows", icon: BookOpenText, records: (shows.data ?? []).map((item) => ({ id: item.id, title: item.title, meta: [item.job_number ? `Job ${item.job_number}` : null, item.client_name, item.location, item.studio_site].filter(Boolean).join(" · ") || "Show", detail: item.summary || item.staffing_notes, href: `/shows/${item.id}` })) },
