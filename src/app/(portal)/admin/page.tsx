...
import { resolveTheme, validPaletteTokens, type CustomPalette } from "@/lib/theme";
...
export default async function AdminPage({
  ...
  const [{ data, error }, { data: settings }, { data: paletteRows }] = await Promise.all([
    ...
    supabase.from("site_settings").select("theme,active_custom_slot").eq("id", "global").maybeSingle(),
    supabase.from("site_custom_palettes").select("slot,name,tokens").order("slot"),
  ]);
  const palettes = (paletteRows ?? []).filter((row) => validPaletteTokens(row.tokens)).map((row) => ({ slot: row.slot, name: row.name, tokens: row.tokens })) as CustomPalette[];
  ...
  return (
    <div className="page-stack">
      ...
      <AppearanceSettings currentTheme={resolveTheme(settings?.theme)} activeCustomSlot={settings?.active_custom_slot ?? null} palettes={palettes} />
      <CustomPalettes key={JSON.stringify(palettes)} saved={palettes} activeSlot={settings?.active_custom_slot ?? null} />
